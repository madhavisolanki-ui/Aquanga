"""
Hybrid CNN + LSTM Deep Learning Training Module for Aquanga
Combines 1D Convolution for feature extraction with LSTM for temporal memory.
"""
...
def build_cnn_lstm_model(input_shape: Tuple[int, int]):
    """Constructs a hybrid 1D CNN + LSTM regression architecture."""
...
def train_cnn_lstm(
    ...
    epochs: int = 60,
    batch_size: int = 4,
    model_dir: str = "models/cnn_lstm"
):
    """Trains and persists the hybrid CNN + LSTM model."""
    import tensorflow as tf
    ...
    logger.info(f"CNN + LSTM model trained and saved to {model_path}")
    return model


def run_cnn_lstm_training() -> Dict[str, Dict[str, float]]:
    """Executes CNN + LSTM model training and evaluation."""
    ...
    model = train_cnn_lstm(X_tr_seq, y_tr_s)
    ...
    results = {
        "CNN + LSTM": {"MAE": mae, "RMSE": rmse, "R2": r2}
    }
    logger.info(f"CNN + LSTM Results: {results}")
    return results
